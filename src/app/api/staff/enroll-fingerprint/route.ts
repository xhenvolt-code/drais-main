...
/**
 * GET /api/staff/enroll-fingerprint?command_id=123
  ...
  try {
    ...
    if (pinMatch) {
      ...
      const fpRows = await query(
        `SELECT id FROM fingerprints f
         JOIN device_user_mappings m ON m.staff_id = f.staff_id
    ...
    console.error('[staff/enroll-fingerprint GET] Error:', err);
...
/**
 * POST /api/staff/enroll-fingerprint
 *
 * Mirrors the student enrollment pattern exactly:
 *   1. Verify device exists → use device.school_id
 *   2. Resolve staff name from people table
 *   3. Look up device_user_mappings by staff_id + device_sn
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { staff_id, device_sn } = body;
    if (!staff_id || !device_sn) {
      return NextResponse.json(
        { error: 'staff_id and device_sn are required' },
        { status: 400 },
      );
    }
    ...
    const safeName = zkName(staffRows[0].first_name, staffRows[0].last_name);

    // ── 3. Check existing device_user_mappings for this staff + device ──
    const mappingRows = await query(
      `SELECT device_user_id FROM device_user_mappings
       WHERE staff_id = ? AND device_sn = ?
       LIMIT 1`,
      [staff_id, device_sn],
    );
    ...
    if (mappingRows && mappingRows.length > 0) {
      ...
      // ── 4a. No mapping → assign next sequential PIN ──
      const maxRow = await query(
        `SELECT MAX(CAST(device_user_id AS UNSIGNED)) AS max_pin FROM device_user_mappings`,
      );
      ...
      // Upsert mapping with staff_id
      await query(
        `INSERT INTO device_user_mappings (school_id, device_user_id, person_type, staff_id, device_sn)
         VALUES (?, ?, 'teacher', ?, ?)
         ON DUPLICATE KEY UPDATE
           staff_id = VALUES(staff_id),
           updated_at = CURRENT_TIMESTAMP`,
        [deviceSchoolId, String(device_user_id), staff_id, device_sn],
      );
    }
    ...
    console.error('[staff/enroll-fingerprint] Error:', err);
    return NextResponse.json(
