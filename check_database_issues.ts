...
async function runDiagnostics() {
  try {
    ...
    if (studentsWithoutAdmission.length > 0) {
      ...
      studentsWithoutAdmission.forEach((student: any, index: number) => {
    ...
    if (deletedStudentsWithResults.length > 0) {
      ...
      deletedStudentsWithResults.forEach((student: any, index: number) => {
    ...
    try {
      ...
      (tableStructure as any[]).forEach((field: any) => {
        console.log(`  - ${field.Field}: ${field.Type} ${field.Null === 'YES' ? '(NULL)' : '(NOT NULL)'}`);
      });
      
      // Check if academic_year_id exists in class_results
      const hasAcademicYear = (tableStructure as any[]).some((field: any) => field.Field === 'academic_year_id');
    ...
    const [stats] = await query(`
      SELECT 
        ...
        (SELECT COUNT(*) FROM class_results WHERE deleted_at IS NULL) as total_results
      FROM students s
    `);
